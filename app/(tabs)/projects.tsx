...
export default function ProjectsScreen() {
  ...
  const fetchProjects = async () => {
    try {
      const res = await api.get('/customer/portal/projects');
              ...
              <Link href={`/project/${p.id}`} key={p.id} asChild>
                <TouchableOpacity style={styles.card}>
                  <View style={styles.cardHeader}>
                    ...
                    <View style={styles.badge}><Text style={styles.badgeText}>{p.status}</Text></View>
                  </View>
                  <Text style={styles.description}>{p.description}</Text>
                  <View style={styles.footer}>
                    <Text style={styles.footerText}>Tap to view board & timeline <Text style={{ fontSize: 16 }}>→</Text></Text>
                  </View>
                </TouchableOpacity>
              </Link>
            ))
...
const styles = StyleSheet.create({
  ...
  header: { padding: 20, paddingTop: 10, borderBottomWidth: 1, borderBottomColor: '#27272a' },
  ...
  projectTitle: { color: '#ffffff', fontSize: 18, fontWeight: 'bold', flex: 1 },
  ...
  footer: { borderTopWidth: 1, borderTopColor: '#27272a', paddingTop: 12 },
  footerText: { color: '#0ea5e9', fontSize: 14, fontWeight: '600' },
});